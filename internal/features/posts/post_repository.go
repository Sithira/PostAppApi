...
const (
	...
	UpdatePostOfUser             = `UPDATE posts SET title = NULLIF($3, ''), body  = NULLIF($4, '') WHERE id = $1 AND user_id = $2 AND deleted_at is NULL;`
...
type PostRepository interface {
	...
	UpdatePostOfUser(ctx context.Context, request entites.Post) (*entites.Post, error)
}
